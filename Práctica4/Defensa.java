...
public class Defensa 
{
	...
	public int[] asignar()
	{
		...
		for(int i=0;i<enemigos.length;i++){
			...
			for(int j=0;j<asignados.length;j++){
				if(asignados[j]==false && grupoDefensaRapida[j]>=enemigos[i]) {
					if(minGanador==-1 || grupoDefensaRapida[j]<grupoDefensaRapida[minGanador]) {
						minGanador=j;
					}
				}if(min==-1 || (asignados[j]==false && grupoDefensaRapida[j]<grupoDefensaRapida[min])) {
					min=j;
				}
			}
			if(minGanador==-1) {
			...
			asignacion[i]=minGanador;
			asignados[minGanador]=true;
		}
		return asignacion;
	}
	...
	public void mostrarPlanDefensaVertical()
	{
		...
		for(int i=0;i<asignados.length;i++)
			System.out.println(asignados[i]);
